...
static const char CopyrString[]= {'G','e','n','e','r','a','t','o','r','e',' ','d','i',' ','b','a','r','r','e',' ',
	'v',VERNUMH+'0','.',VERNUML/10+'0',(VERNUML % 10)+'0', ' ','1','5','/','1','1','/','2','3', 0 };
...
#else
#warning dovremmo avere il clock a 142MHz per Chroma... sintonizzare, tarare...
/*	
  Fosc= Fin*M/(N1*N2)
	0.8MHz<(Fin/N1)<8MHz
    ...
    if(!sw1) {
      ...
      if(!(oldSw & 1)) {
        if(swPressed>=TIME_TO_LONG_CLICK) {
          configParms.Reverse = !configParms.Reverse;			// vabbe' 
          }
        else {
