...
#define DEE_PAGE_OFFSET(bank, page) ((0x7FC000 + (DEE_BANK_SIZE * (bank)) + (DEE_PAGE_SIZE * (page))) & 0xFFFF)
    ...
    unsigned char emulationPages[DATA_EE_BANKS * NUM_DATA_EE_PAGES][NUMBER_OF_INSTRUCTIONS_IN_PAGE /* direi di no.. * 2*/]
    // io avrei anche invertito le 2 dimensioni ma poi non va... boh
        __attribute__((space(psv), aligned(NUMBER_OF_INSTRUCTIONS_IN_PAGE * 2), noload));
// da dee_emulation_pic32.c ... const unsigned int eedata_addr[NUM_DATA_EE_PAGES][NUMBER_OF_INSTRUCTIONS_IN_PAGE] __attribute__ ((aligned(4096)))={0};
...
#if defined(__dsPIC33E__) || defined(__PIC24E__) || defined(__dsPIC33C__)     // ok dsPIC33CH 15/12/19
